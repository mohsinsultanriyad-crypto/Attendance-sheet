...
const EntriesList: React.FC<{ onNavigate: (view: any, param?: string) => void }> = ({ onNavigate }) => {
  ...
  const loadData = async () => {
    ...
    const allEntries = await sheetGetAll();
    ...
    const filtered = allEntries
      ...
      .map((e: any) => ({
        ...e,
        id: String(e.id), // sheet id
        workerId: Number(e.workerId),
        workingHours: Number(e.workingHours || 0),
        otHours: Number(e.otHours || 0),
        isRejectedLeave: Boolean(e.isRejectedLeave),
        isApprovedLeave: Boolean(e.isApprovedLeave),
        notes: String(e.notes || ""),
        workerName: allWorkers.find((w) => w.id === Number(e.workerId))?.name || `Worker #${e.workerId}`,
      }))
  ...
  const deleteEntry = async (id: string) => {
    if (confirm("Delete this entry (shared)?")) {
  ...
  const totalHours = entries.reduce((acc: number, curr: any) => acc + Number(curr.workingHours || 0), 0);
  const totalOT = entries.reduce((acc: number, curr: any) => acc + Number(curr.otHours || 0), 0);
          ...
          entries.map((e: any) => (
            <div
              ...
              className={`bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3 ${
                e.isRejectedLeave ? "border-l-4 border-l-red-500" : e.isApprovedLeave ? "border-l-4 border-l-emerald-500" : ""
              }`}
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-xs font-bold text-blue-600 uppercase tracking-tighter mb-1">
                    {format(parseISO(String(e.date)), "EEE, MMM d, yyyy")}
                  </div>
                  <h3 className="font-black text-slate-900">{e.workerName}</h3>
                </div>

                <div className="flex gap-1">
                  <button onClick={() => onNavigate("add-entry", String(e.id))} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg">
                    <Edit3 size={18} />
                  </button>
                  <button onClick={() => deleteEntry(String(e.id))} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs font-bold">
                {e.isRejectedLeave ? (
                  ...
                  <>
                    ...
                    <div className="bg-blue-50 text-blue-700 px-3 py-2 rounded-lg border border-blue-100">{e.workingHours}h</div>
                  </>
                )}

                {Number(e.otHours) > 0 && !e.isRejectedLeave && !e.isApprovedLeave && (
                  <div className="bg-emerald-50 text-emerald-700 px-3 py-2 rounded-lg border border-emerald-100">OT {e.otHours}h</div>
                )}
              </div>

              {e.notes && <p className="text-[11px] text-slate-500 italic bg-slate-50 p-2 rounded-lg">"{e.notes}"</p>}
            </div>
          ))
        )}
      </div>
