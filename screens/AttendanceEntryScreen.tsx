...
const AttendanceEntryScreen: React.FC<{ onNavigate: (v: any) => void; entryId?: string }> = ({ onNavigate, entryId }) => {
  ...
  useEffect(() => {
    loadWorkers();
    if (entryId) loadExistingEntryById(parseInt(entryId));
  }, [entryId]);
  ...
  const loadExistingEntryById = async (id: number) => {
  ...
  const applyEntryToForm = (entry: AttendanceEntry) => {
    ...
    if (!entry.isRejectedLeave && !entry.isApprovedLeave) {
      ...
      setBreakMin(entry.breakMinutes);
    }
    setIsRejectedLeave(entry.isRejectedLeave || false);
    setIsApprovedLeave(entry.isApprovedLeave || false);
    setAdvancePayment(entry.advancePayment || 0);
  ...
  useEffect(() => {
    ...
    const checkData = async () => {
      ...
      const existing = await db.entries.where({ workerId: selectedWorkerId, date }).first();
      ...
      if (existing) {
        ...
        const latest = await db.entries
          .where('workerId')
          .equals(selectedWorkerId)
          .reverse()
          .sortBy('date');
        
        const lastValid = latest.find(e => !e.isRejectedLeave && !e.isApprovedLeave);
        
        if (lastValid) {
          setCheckIn(lastValid.checkIn);
          setCheckOut(lastValid.checkOut);
          setBreakMin(lastValid.breakMinutes);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existing) {
        await db.entries.update(existing.id!, payload);
      } else {
      ...
      <form onSubmit={handleSave} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button 
            ...
            className={`px-3 py-2.5 rounded-xl text-[10px] font-black uppercase transition-colors flex items-center justify-center gap-2 border ${isApprovedLeave ? 'bg-emerald-600 border-emerald-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-2.5 rounded-xl text-[10px] font-black uppercase transition-colors flex items-center justify-center gap-2 border ${isRejectedLeave ? 'bg-red-600 border-red-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100'}`}
          >
            <Ban size={14} />
        ...
        <div>
          ...
          <select value={selectedWorkerId} onChange={(e) => setSelectedWorkerId(parseInt(e.target.value))} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" required>
        ...
        <div>
          ...
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" required />
        </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" placeholder="7 AM" value={checkIn} onChange={(e) => { setCheckIn(e.target.value); setIsDataSuggested(false); }} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" required />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Check Out</label>
                <input type="text" placeholder="5 PM" value={checkOut} onChange={(e) => { setCheckOut(e.target.value); setIsDataSuggested(false); }} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" required />
              </div>
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Break (Minutes)</label>
              <select value={breakMin} onChange={(e) => setBreakMin(parseInt(e.target.value))} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none">
        ...
        {isApprovedLeave && (
          <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl text-center">
            <div className="text-xs font-black text-emerald-600 uppercase">Approved Leave</div>
            <div className="text-[10px] text-emerald-500 font-bold mt-1">This is a valid leave. No salary will be cut.</div>
          </div>
        )}

        {isRejectedLeave && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-2xl text-center">
            <div className="text-xs font-black text-red-600 uppercase">Warning: Deduction Enabled</div>
            <div className="text-[10px] text-red-500 font-bold mt-1">Rejected Leave: This will deduct 1 day's pay from final salary.</div>
          </div>
        )}

        {preview && !isRejectedLeave && !isApprovedLeave && (
          <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex justify-between items-center">
            <div>
              <div className="text-[10px] font-bold text-blue-600 uppercase">Working Hours</div>
              <div className="text-xl font-black text-blue-900">{preview.working}h</div>
            </div>
            <div className="text-right">
              <div className="text-[10px] font-bold text-blue-600 uppercase">OT Pay (Estimate)</div>
              <div className="text-xl font-black text-blue-900">₹{preview.otPay}</div>
            </div>
          </div>
        )}
