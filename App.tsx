...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (view) {
        ...
        if (rest.length > 0) {
          // simple param parsing for edits
          setParams({ id: rest[0] });
        }
      } else {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'workers': return <WorkersList onNavigate={navigate} />;
      case 'add-entry': return <AttendanceEntryScreen onNavigate={navigate} entryId={params.id} />;
      case 'entries': return <EntriesList onNavigate={navigate} />;
